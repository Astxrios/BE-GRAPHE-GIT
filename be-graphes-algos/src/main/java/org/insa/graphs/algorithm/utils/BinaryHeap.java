...
public class BinaryHeap<E extends Comparable<E>> implements PriorityQueue<E> {
    ...
    public void remove(E x) throws ElementNotFoundException {
        if (this.isEmpty() || this.array.indexOf(x)<0 || this.array.indexOf(x)>this.currentSize-1){
            throw new ElementNotFoundException(x);
        }
        int pos = this.array.indexOf(x);
        if (pos==this.currentSize-1){
            currentSize--;
            return;
        }
        this.array.set(pos,this.array.get(--this.currentSize));
        percolateUp(pos);
        percolateDown(pos);
